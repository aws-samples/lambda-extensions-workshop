...
def start_http_listener(queue):
    ...
    print ("[telemetry_http_listener.start_http_listener] Starting http listener on {0}:{1}".format(LISTENER_ADDRESS, LISTENER_PORT))    
    ...
    if not is_event_started:
        print ("[telemetry_http_listener.start_http_listener] server_thread has timedout before starting")
        raise Exception("server_thread has timedout before starting")

    print ("[telemetry_http_listener.start_http_listener] Started http listener")    
...
class TelemetryRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            print("in do_POST, batch: " + str(batch))
